...
// Mobile View Right TOC
	...
	rightTocObj.find('.hide-icon').css("display","none");
	rightTocObj.find('.show-icon').css("display","block");	
	}
});
//Right TOC show and Hide

$(document).on("click", ".hide-icon", function (event) {	
	var rightTocIcon=$('.right-toc-icon');
	rightTocIcon.css({"width":"50px","transition":"width 0.5s"});	
...
$(document).on("click", ".show-icon", function (event) {
	...
	rightTocIcon.css({"width":"250px","transition":"width 0.5s"});	
